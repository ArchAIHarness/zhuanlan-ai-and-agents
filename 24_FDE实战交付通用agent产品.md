...
但我心里清楚：这个脚本换个部门就用不了。每个部门的工作流都不一样。
...
"我不管你怎么弄的，我也要一个 AI，帮每家门店做员工培训考核——每家菜品不一样、标准不一样，你得一个店一个店来。"

两个需求完全不同。法务是"从合同里抽数据"，培训是"给每家出考核题"。但有一件事是一样的：**AI 都得跑在客户的电脑上**。
...
我用了三天写了一个 gateway——一个轻量的调度层。它做三件事：
