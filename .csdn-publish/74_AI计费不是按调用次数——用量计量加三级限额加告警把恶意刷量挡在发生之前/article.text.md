73 期讲了张磊凌晨 4 点 13 分那个 8000 美元账单。
...
![第1节配图](assets/74_usage_metering_three_tier_limit/01-精确计量仪表盘.png)
## §一 反差开场——从单晚 8000 美元到 3 个月零超账单
先把张磊整改后的数字摆出来。
73 期讲张磊凌晨 4 点 13 分单晚烧光 8000 美元、月度账单 8.2 万、年度差点融资砍半。这是 6 个月前的数字。
6 个月后——
...
![第2节配图](assets/74_usage_metering_three_tier_limit/02-三级限额架构.png)
...
- **电话**：重大事故（1 分钟内人工接通）
- **IM 群**：实时同步（10 秒内）
**73 期讲过张磊手机静音模式淹没告警**——这是张磊复盘后改的：他给自己配了短信 + IM 群双通道，邮件只发给团队其他人。
### 4.3 一个反常识洞察
...
![第3节配图](assets/74_usage_metering_three_tier_limit/03-实时告警时序.png)
...
为什么？
因为 73 期讲过的三类异常（恶意刷量、业务突增、配置错误），**业务突增和配置错误都不是"坏人"**——它们是正常用户 / 正常运营导致的。但它们的账单放大效应跟恶意刷量一样。
...
**五层防护不是"装 5 个工具"，是"装 5 个不同视角的监控"——总有一个视角能看到异常。**
![第4节配图](assets/74_usage_metering_three_tier_limit/04-异常检测雷达图.png)
...
**账单可视化是 AI 时代 SaaS 的"信任基础设施"——没有它，所有 AI 计费都是空中楼阁。**
![第5节配图](assets/74_usage_metering_three_tier_limit/05-final-quote金句.png)
...
**这一期和 73 期的反常识认知，是把"AI 计费 = 电力消耗"翻译成"AI 计费 = 装电表 + 配限额 + 设告警 + 抓异常"。**
...
**AI 计费不是按调用次数，是把每一笔 token 算到能看见、能拦住、能退。**
这一期的核心，是把 73 期的反常识认知"AI 计费 = 装电表"翻译成可落地的工程纪律——
- **能看见**：精确计量 + 用量打点 + 实时聚合
- **能拦住**：三级限额 + 三档告警 + 五层防护
...
---
## 73 + 74 双篇收束
73 期我们讲了"AI 计费 = 装电表"的心智模型——账单爆炸的本质是"没装电表"，限额不是省钱是救命。
74 期我们讲了"AI 计费 = 装电表 + 配限额 + 设告警 + 抓异常"的工程纪律——精确计量 / 三级限额 / 三档告警 / 四类异常检测 / 五层防护 / 多租户分摊 / 账单可视化。
