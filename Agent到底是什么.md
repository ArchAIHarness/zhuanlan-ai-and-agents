...
![聊天模型回答问题，Agent 推进任务](assets/Agent到底是什么/02-comparison-chatbot-vs-agent.png)
...
![Agent 的目标、计划、工具、观察、修正和验证闭环](assets/Agent到底是什么/03-flowchart-agent-loop.png)
...
![Agent 工具权限边界：只读、写入、危险动作](assets/Agent到底是什么/04-framework-tool-permission-boundary.png)
...
![Agent 常见翻车原因：目标不清、工具乱用、缺少验证、权限过大](assets/Agent到底是什么/05-infographic-agent-failure-causes.png)
