...
![Spec 端手册挂在墙上,新人(AI 小厨子)读手册一次吸收成自己的习惯](assets/31_SDD完整指南/01-spec-manual-on-wall.png)
...
![Spec 端沉淀的六大类团队技术范式,人执笔更新、AI 闭眼吸收记忆](assets/31_SDD完整指南/02-spec-content-decomposition.png)
...
![工程师(服务员)只对顾客点菜单、转身交给厨房,AI 小厨子自动按墙上 Spec 手册匹配"牛排 7 分熟"标准做法落地做菜](assets/31_SDD完整指南/03-story-order-and-ai-cooks.png)
...
![厨房平面图:三个刚性约束区(DDD 工种分区 / TDD 试菜员 / Spec 墙上挂手册)之间的留白区,AI 小机器人正在自由决定葱花怎么摆、盘边怎么装饰](assets/31_SDD完整指南/04-leaving-blank-area-map.png)
...
![SDD 完整四阶段工作流:前置工程(平台搭建)→ 需求对话(Story 端)→ AI 自动开发(Spec 基线匹配)→ TDD 全流程审计(自动拦截)→ 上线交付(灰度/监控/回滚),五个节点每个都标注责任人 + 关键工具](assets/31_SDD完整指南/05-four-stage-workflow.png)
